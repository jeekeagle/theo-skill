---
name: theo-card
description: "Content caster (铸). Transforms content into PNG visuals. Three molds: -l (default) long reading card, -i infograph, -c multi-card reading cards (1080x1440). Output to ~/Downloads/. Use when user says '铸', 'cast', '做成图', '做成卡片', '做成信息图', '做成海报'. Replaces theo-cards and theo-infograph."
user_invocable: true
version: "1.6.0"
---
...
```bash
node ~/.claude/skills/theo-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.claude/skills/theo-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.claude/skills/theo-card && npm install playwright && npx playwright install chromium
```
