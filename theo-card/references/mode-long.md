...
Read `~/.claude/skills/theo-card/assets/long_template.html`
...
| `{{SOURCE}}` | 来源/作者信息（用户提供则填入，否则 `Theo`） |
| `{{ARXIV_LINE}}` | 内容来自 arxiv 论文时：`<span class="arxiv">arxiv: XXXX.XXXXX</span>`；否则空字符串 |

写入：`/tmp/theo_cast_long_{name}.html`

## 步骤 5：截图

```bash
node ~/.claude/skills/theo-card/assets/capture.js /tmp/theo_cast_long_{name}.html ~/Downloads/{name}.png 1080 800 fullpage
```
